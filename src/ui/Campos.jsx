...
export function Campo({ label, children, ancho = "" }) {
  return (
    <label className={`block ${ancho}`}>
      <span className="text-[10px] uppercase tracking-widest text-texto-tenue font-bold">{label}</span>
      {children}
    </label>
  );
}

export const inputCls = "w-full border border-borde rounded-lg px-2.5 py-1.5 text-sm mt-1 outline-none focus:border-acento";
